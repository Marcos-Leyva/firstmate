...
Those same two verdicts are what let a state read refuse to report a killed worker as still working; the semantic busy-state contract owned by `bin/fm-busy-lib.sh` owns that override.
