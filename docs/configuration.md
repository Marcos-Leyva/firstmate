...
kiro is verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate, because Kiro CLI has no primary supervision protocol (`asyncRewake`, hook-driven continuation); the harness adapter reference at [`.agents/skills/harness-adapters/references/harness/kiro.md`](../.agents/skills/harness-adapters/references/harness/kiro.md) owns that evidence.
