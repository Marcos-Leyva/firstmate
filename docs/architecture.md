...
A crew that declares `paused:` for a known external wait, or carries a verified `captain-held` transfer, is separately absorbed and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
When a declared pause coexists with a provably-working run, the pause wins and the pane takes the bounded pause cadence instead of the wedge timer, because the worker said it is waiting and the active run confirms the pipeline is alive.
...
Fresh stale panes use the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation, except that a declared `paused:` wait outranks the active run and keeps the pause cadence.
...
Only when no matching run exists does it consult semantic busy state; exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or dead (a gone endpoint or a proven-absent agent) stays unknown instead of trusting a stale log.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, kiro, cursor, and muse while preserving the requested profile for later audit.
