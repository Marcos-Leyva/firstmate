...
| Skill | No `/skill` command. Standing instructions ride the agent config's `prompt` (`file://` URI) and `resources` (glob) fields. The no-mistakes procedure is embedded in the agent config at spawn time rather than invoked interactively. |
